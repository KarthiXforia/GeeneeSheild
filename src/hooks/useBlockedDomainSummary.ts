// src/hooks/useBlockedDomainSummary.ts
import { useEffect, useRef } from "react";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import type { ApiError } from "@/lib/api/client";
import {
  fetchBlockedDomainSummary,
  fetchBlockedDomainSummaryForSuspense,
} from "@/lib/api/domains";
import { useBlockedDomainStore } from "@/store/blockedDomainStore";

// Define query key for reuse in prefetching
export const BLOCKED_DOMAIN_SUMMARY_QUERY_KEY = ["blockedDomainSummary"];

export function useBlockedDomainSummary() {
  ...
  const query = useQuery({
    queryKey: BLOCKED_DOMAIN_SUMMARY_QUERY_KEY,
    queryFn: async () => {
      ...
      try {
        return await fetchBlockedDomainSummary({ signal: controller.signal });
      } catch (error) {
        const apiError = error as ApiError;
        // Convert to a format React Query can handle
        throw new Error(apiError.message || "Failed to fetch blocked domain summary");
      }
    },
...
/**
 * Custom hook for prefetching blocked domain summary data
 * @returns Object with prefetch function
 */
export function usePrefetchBlockedDomainSummary() {
  const queryClient = useQueryClient();
  const store = useBlockedDomainStore();

  const prefetchBlockedDomainSummary = async () => {
    // Only prefetch if we don't have fresh data
    if (!store.summary.isFresh) {
      await queryClient.prefetchQuery({
        queryKey: BLOCKED_DOMAIN_SUMMARY_QUERY_KEY,
        queryFn: fetchBlockedDomainSummaryForSuspense,
      });
    }
  };

  return { prefetchBlockedDomainSummary };
}
