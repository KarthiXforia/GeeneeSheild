// src/hooks/useBlockedDomainHeatmap.ts
import { format, subDays, subMonths, subWeeks } from "date-fns";
import { useEffect, useMemo, useRef } from "react";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import type { ApiError } from "@/lib/api/client";
import { fetchBlockedDomainHeatmap } from "@/lib/api/domains";
import { useBlockedDomainStore } from "@/store/blockedDomainStore";
import type { TimeframeOption } from "@/types/domains";

// Define query key for reuse in prefetching
export const BLOCKED_DOMAIN_HEATMAP_QUERY_KEY = ["blockedDomainHeatmap"];
 ...
 */
function getDateRangeForTimeframe(timeframe: TimeframeOption): {
  startDate: string;
  endDate: string;
} {
...
export function useBlockedDomainHeatmap({
  period,
  customStartDate,
  customEndDate,
}: {
  ...
  const query = useQuery({
    queryKey: [...BLOCKED_DOMAIN_HEATMAP_QUERY_KEY, period, startDate, endDate],
    queryFn: async () => {
      ...
      try {
        return await fetchBlockedDomainHeatmap(
        ...
        throw new Error(
          apiError.message || "Failed to fetch blocked domain heatmap data",
        );
      }
    },
    ...
    initialData: isFresh ? store.heatmapData : undefined,
  ...
  useEffect(() => {
    ...
    if (
      ...
      JSON.stringify(query.data) !== JSON.stringify(store.heatmapData)
    ) {
      hasUpdatedStore.current = true;
      store.setHeatmapData(query.data);
...
export function usePrefetchBlockedDomainHeatmap() {
  ...
  return (params: {
    ...
    return queryClient.prefetchQuery({
      queryKey: [
        ...BLOCKED_DOMAIN_HEATMAP_QUERY_KEY,
        params.period,
        startDate,
        endDate,
      ],
      queryFn: () =>
        fetchBlockedDomainHeatmap(
          {
