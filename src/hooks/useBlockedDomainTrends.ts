// src/hooks/useBlockedDomainTrends.ts
import { format, subDays, subMonths, subWeeks } from "date-fns";
import { useEffect, useMemo, useRef } from "react";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import type { ApiError } from "@/lib/api/client";
import { fetchBlockedDomainTrends } from "@/lib/api/domains";
...
export const BLOCKED_DOMAIN_TRENDS_QUERY_KEY = ["blockedDomainTrends"];
...
export function useBlockedDomainTrends({
  period,
  ...
  const isFresh = useMemo(() => {
    return (
      store.trends.lastUpdated &&
      Date.now() - store.trends.lastUpdated < 60 * 60 * 1000 && // 1 hour cache
      store.trends.params?.period === period
    );
  }, [store.trends.lastUpdated, store.trends.params?.period, period]);

  const query = useQuery({
    queryKey: [...BLOCKED_DOMAIN_TRENDS_QUERY_KEY, period, startDate, endDate],
    queryFn: async () => {
      ...
      try {
        ...
        const data = await fetchBlockedDomainTrends(
        ...
        store.setTrendsError(
          apiError.message || "Failed to fetch blocked domain trends",
        );
        throw error;
      } finally {
...
export function usePrefetchBlockedDomainTrends() {
  const queryClient = useQueryClient();

  return (params: BlockedDomainTrendsParams) => {
    const { startDate, endDate } = getDateRangeForTimeframe(params.period);

    return queryClient.prefetchQuery({
      queryKey: [
        ...BLOCKED_DOMAIN_TRENDS_QUERY_KEY,
        params.period,
        startDate,
        endDate,
      ],
      queryFn: () =>
        fetchBlockedDomainTrends(
          {
